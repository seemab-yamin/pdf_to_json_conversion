...
def extract_data(table):
    ...
    for i in df.columns:
        if all(df.applymap(lambda x: x == "")[i]):
...
if __name__ == "__main__":
    ...
    for i, table in enumerate(tables):
        ...
        try:
            ...
            tables_skipped_counter += 1
            print(f"\n{e}\n")
            print(f"Skipping Invalid Table Structure:\t{i+1}")

    if tables_skipped_counter:
        print(f"Total Bad Tables Skipped:\t{tables_skipped_counter}")
